...
`ai/prompt_builder._render_candidates`, which interpolates publisher-controlled title/body
verbatim), so it has been removed rather than left as a footgun for the next caller.

`run_data_only_prompt` must be verifiably tool-free: no bash, no filesystem, no MCP, no ask_user
-- a prompt-injection payload hidden in fetched content must not be able to reach a single tool.
It proves this by passing `available_tools=[]`, which the SDK docs
(`CopilotClient.create_session`) state takes precedence over every other tool source (built-in,
MCP, custom) and, empty, leaves nothing available. Belt-and-braces on top of that allowlist:
`on_permission_request` denies (never approves) and `on_user_input_request` raises rather than
answering, so that if some future SDK version ever routes a capability outside the
`available_tools` gate, this fails loudly instead of quietly behaving like `approve_all`. Before
any of that, `_require_tool_free_capability` inspects the installed SDK's `create_session`
signature for the `available_tools` parameter; if a future or older SDK release doesn't expose
it, this raises immediately -- callers must never fall back to an ungated, tool-permissive
session.
